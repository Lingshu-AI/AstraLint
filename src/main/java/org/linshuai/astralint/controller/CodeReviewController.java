...
public class CodeReviewController {
     ...
     */
    @PostMapping("/submit")
    public ResponseEntity<CodeReviewResponse> submitCodeReview(@Valid @RequestBody CodeReviewRequest request) {
        try {
            logger.info("收到代码审查请求: projectId={}, mergeRequestId={}",
                    request.getProjectId(), request.getMergeRequestId());

            String reviewId = UUID.randomUUID().toString();
            CodeReviewResponse response = new CodeReviewResponse(reviewId, request.getProjectId(),
                    request.getMergeRequestId());
                ...
                try {
                    ...
                    switch (request.getReviewType()) {
                        case SECURITY:
                            ...
                            break;
                        case PERFORMANCE:
                            analysisResult = aiCodeReviewService
                                    .provideOptimizationSuggestions(request.getDiffContent());
                            break;
                        case BASIC:
     ...
     */
    @PostMapping("/quick")
    public ResponseEntity<String> quickCodeReview(@Valid @RequestBody CodeReviewRequest request) {
        try {
    ...
    public ResponseEntity<String> securityCheck(@Valid @RequestBody CodeReviewRequest request) {
        try {
    ...
    public ResponseEntity<String> performanceOptimization(@Valid @RequestBody CodeReviewRequest request) {
        try {
     ...
     */
    @PostMapping("/summary")
    public ResponseEntity<String> generateSummary(@Valid @RequestBody CodeReviewRequest request) {
        try {
